...
router.get('/', function (req, res, next) {
  ...
  const titleQ = queries.title ? String(queries.title).trim() : '';
  const slugQ = queries.slug ? String(queries.slug).trim() : null;
  const minPrice = queries.minPrice !== undefined ? Number(queries.minPrice) : 0;
  const maxPrice = queries.maxPrice !== undefined ? Number(queries.maxPrice) : 1e6;
  const page = queries.page !== undefined ? Number(queries.page) : 1;
  const limit = queries.limit !== undefined ? Number(queries.limit) : 10;
  ...
  if (slugQ) {
    result = result.filter(e => e.slug === slugQ);
  } else if (titleQ) {
    const tq = titleQ.toLowerCase();
    result = result.filter(e => e.title.toLowerCase().includes(tq));
  }
...
router.post('/', function (req, res, next) {
  ...
  const newObj = {
    ...
    slug: ConvertTitleToSlug(String(title)),
    price: Number(price),
...
router.put('/:id', function (req, res, next) {
  ...
  const allowedKeys = ['title','price','description','category','images','slug'];
